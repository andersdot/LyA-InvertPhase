...
n = 50
snapshot_dir_pre = '/home/fvillaescusa/data/Lya_ncv/20Mpc_256/'
...
for sim_pre in sim_pres:
    ...
    for snapshot_number in snapshot_numbers:
        ...
        for simulation_number, c in zip(simulation_numbers, colors):
            ...
            fourier_estimator_instance = fou.FourierEstimator1D(delta_flux_box)
            power = fourier_estimator_instance.get_power_1D()
            k = simulation_box_instance.k_i('z').value
            #np.savez(SPECTRA_SAVEDIR + POWER_SPECTRA_SAVEFILE, power_binned, k_binned, mu_binned, bin_counts)

            p1.append(power)
            k1.append(k)
            #plt.plot(np.log10(k), np.log10(power), color=c, label=None)
            #plt.xlabel('k [h/Mpc]')
            #plt.ylabel('P(k)')
            #plt.tight_layout()
            #plt.savefig('test{0}.pdf'.format(sim_pre))
            #import pdb; pdb.set_trace()
        np.savez('spec1d{0}_{1}{2}_1mubin_20Mpc'.format(grid_width, sim_pre, snapshot_number), power=p1, k=k1)

    #plt.legend(loc='best')
    #plt.xlabel('k [h/Mpc]')
    #plt.ylabel('P(k)')
    #plt.tight_layout()
    #plt.savefig('test1d{0}.pdf'.format(sim_pre))
