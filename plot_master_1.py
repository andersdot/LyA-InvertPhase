...
def create_mean_std(bigbox, dimension, sn, fixed=False):
    ...
    return kT[0], meanT, np.sqrt(varT), kP[0], meanP, np.sqrt(varP)
...
def make_plot(power, zz, colors, snaps, fout,f_1,f_2,fr, bigbox, dimension,
              ...
              avoid_z=False):
    plt.clf()
    fig=figure(figsize=(9/1.4,20/1.4))

    ########### create plots and subplots ############
    gs = gridspec.GridSpec(4,1,height_ratios=[4,2,2,4])
    ...
    if power == 'lya':
        ...
        ax[3].set_ylabel('$\mathrm{\sigma_T^2/\sigma_P^2}$', fontsize=12)
        ax[3].set_xlabel('$\mathrm{k_{\parallel} \; [h/Mpc]}$', fontsize=12)

        ax[1].axhline(0, linestyle='--', color='black')
        #ax[1].axhline(-3, linestyle='--', color='black', lw=0.5)
        #ax[1].axhline(3, linestyle='--', color='black', lw=0.5)

        ax[3].axhline(1, linestyle='--', color='black')

        if dimension == '1d':
            ax[3].set_xlabel('$\mathrm{k_{\parallel} \; [h/Mpc]}$', fontsize=12)

        else:
            ax[3].set_xlabel('$\mathrm{k \;[h/Mpc]}$', fontsize=12)

    ax[1].set_ylim(-y_lim_2, y_lim_2)
    ax[2].set_ylim(-y_lim_3, y_lim_3)
    ax[3].set_ylim(0.5, y_max_4)
    ax[3].set_yscale('log')
    ...
    ax[1].set_ylabel(r'$\mathrm{\Delta \overline{P}/\overline{P}_T\; [\%]}$')
    ax[2].set_ylabel(r'$\mathrm{\Delta \overline{P}/\sigma}$')

    ax[3].set_yscale('log')
    ax[0].set_yscale('log')
    ax[0].set_ylim(y_min_1, y_max_1)
    ...
    for z, sn, c in zip(zz, snaps, colors):
        ...
        #import pdb; pdb.set_trace()
        ax3.plot(k1, (Pk1-Pk2)/np.sqrt(dPk1**2/nstandard + dPk2**2/npaired), color=c)
        ax3.axhline(0.0, color='k', linestyle = '--', zorder=-1)
        ...
        ax4.plot(k1, ratio, c=c,lw=2)

        if power == 'lya':
            ax4.plot(k1, (dPk1/dPkF)**2., c=c, lw=2, linestyle='--')
            p5,=ax4.plot(k1, np.zeros(len(k1))+1e5, c='k', lw=2, linestyle='--')
        p4,=ax4.plot(k1, np.zeros(len(k1))+1e5, c='k', lw=2)
        ax4.fill_between(k1, ratio+dratio, ratio-dratio, color=c,alpha=0.2)
        if power == 'matter':
            ...
            ax4.plot(k_r, ratio_lazy, c=c,lw=2, linestyle='--')
            p5,=ax4.plot(k_r, np.zeros(len(k_r))+1e5, c='k', lw=2, linestyle='--')
        ...
        ax4.plot([x_min, x_max],[1.0, 1.0], c='k',linestyle=':')
    for a in ax:
    ...
    print(ls, hs)
    ax2.legend(ls, hs, ncol=3, bbox_to_anchor=(0.6, 2.0))
