...
for sim_pre in sim_pres:
    ...
    for snapshot_number in snapshot_numbers:
        ...
        for simulation_number, c in zip(simulation_numbers, colors):
            ...
            k_bin_edges = k_bin_edges = np.load('20Mpc_hydro/20Mpc_kvalues.txt')
            ...
            plt.ylabel('P(k)')
            plt.tight_layout()
            plt.savefig('test{0}_25Mpc.pdf'.format(sim_pre))
            #import pdb; pdb.set_trace()
        np.savez('spec3d{0}_{1}{2}_1mubin_20Mpc'.format(grid_width, sim_pre, snapshot_number), power=p1, k=k1, mu=mu1)
